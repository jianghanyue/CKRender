...
import { SkyBaseGroup, SkyBaseLayer, SkyModel } from './model'
import { SkyBasePathView, SkyView } from './view'
...
export class EditorState {
  ...
  _model: SkyModel = new SkyModel()
  viewRef: SkyView | undefined
  ...
  private outlineChangeEvent = 0

  outlineListRef = () => {
    if (this.outlineChangeEvent < 0) return []
    const model = this._model
    const page = model?.pages[this.selectedPageIndex]
    if (page) {
  ...
  get selectedPageModel() {
    return this.model?.pages[this.selectedPageIndex]
  }

  get selectedLayerView() {
    return this.view?.getViewByModelId(this.selectedLayerIdRef)
  }
  ...
  async openSketchArrayBuffer(filename: string, arrayBuffer: ArrayBuffer, el: HTMLElement) {
    let zipFile: JSZip
    console.log(filename,arrayBuffer,el,'err')
  ...
  initBinding(view: SkyView) {
    ...
    this.bindings.push(
      view.pageState.selectionChange.subscribe(() => {
        this.selectedLayerIdRef = view.pageState.selectedLayerView?.model.objectId ?? ''
        if (this.selectedLayerModel) {
          this.selectedPageModel?.expandLayers(this.selectedLayerModel)
        }
        this.outlineChangeEvent++
      }),
      view.pageState.hoverChange.subscribe(() => {
        this.hoveredLayerIdRef = view.pageState.hoverLayerView?.model.objectId ?? ''
      })
    )
  }
...
//
// function rxToRef<T>(ob: Observable<T>): T | undefined {
//   const ret: <T | undefined> = ref()
...
function refToRx<T>(skyView: T) {
  const sb = new BehaviorSubject<T>(skyView)
  return sb
}
