...
import { SkyBaseLayerView, SkyPageView, OverlayView, SkyArtboardView } from '.'
...
export class SkyView extends Disposable {
  ...
  private debugPrintTree(node: any = this.pageView) {
    ...
    node?.children?.forEach((child: any) => this.debugPrintTree(child))
    console.groupEnd()
  }
  ...
  getImgFromSkImage(image: SkImage) {
    ...
    if (!context) throw Error("context is not create")
    if (!data) throw Error("data is not create")
